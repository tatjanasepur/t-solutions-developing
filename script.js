...
function wireForm(formId,noteId){
  const f=document.getElementById(formId); const note=document.getElementById(noteId);
  if(!f) return;
  f.addEventListener('submit',e=>{
    ...
    const subj=encodeURIComponent(`Upit sa sajta — ${decodeURIComponent(name)}`);
    const body=encodeURIComponent(`Ime i prezime: ${decodeURIComponent(name)}%0AEmail: ${decodeURIComponent(email)}%0A%0APoruka:%0A${decodeURIComponent(msg)}`);
